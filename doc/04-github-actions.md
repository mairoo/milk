...
GitHub `milk` 저장소에서 Settings > Security > Secrets and variables > Actions > Repository secrets

- **PINCOIN_DOT_ENV**: .env 파일 내용 전체
- **PREFIX**: `pincoin`
...
```bash
sudo systemctl status actions.runner.mairoo-coconut.pincoin-frontend-prod-runner.service
```
