...
/**
 * NextAuth.js 인증 설정
 * Keycloak과 연동하여 OAuth 2.0/OpenID Connect 인증 처리
 * JWT 기반 세션 관리와 토큰 자동 갱신 지원
 */
export const authOptions: NextAuthOptions = {
    /**
     * 인증 제공자 설정
     * Keycloak OAuth 2.0/OpenID Connect 제공자를 설정
     */
    providers: [
        KeycloakProvider({
    ...
    /**
     * 세션 관리 설정
     * JWT 기반 세션을 사용하여 서버 메모리 사용량을 최적화
     * 데이터베이스 없이도 확장 가능한 세션 관리 가능
     */
    session: {
        strategy: 'jwt',
    ...
    /**
     * 콜백 함수 설정
     * 인증 과정에서 JWT 토큰과 세션 데이터를 커스터마이징
     */
    callbacks: {
        /**
         * JWT 콜백 - 토큰 생성 및 갱신 처리
         * Keycloak에서 받은 액세스 토큰, 리프레시 토큰, 사용자 정보를 JWT에 저장
         ...
         * JWT에서 필요한 정보만 추출하여 세션 객체를 구성
         * 보안상 민감한 토큰 정보는 서버 사이드에서만 사용
         */
    ...
    /**
     * 커스텀 페이지 설정
     * NextAuth.js의 기본 페이지 대신 커스텀 페이지를 사용
     */
    pages: {
    ...
    /**
     * 이벤트 핸들러 설정
     * 인증 관련 이벤트 발생 시 추가 처리를 수행
     */
    events: {
        /**
         * 로그아웃 이벤트 핸들러
         * NextAuth.js 로그아웃 시 Keycloak 서버에서도 완전히 로그아웃
         */
        async signOut({token}) {
            // Keycloak 로그아웃 처리
            if (token.idToken) {
 ...
 *
 * Keycloak의 리프레시 토큰을 사용하여 만료된 액세스 토큰을 새로 발급 받음
 * 토큰 갱신에 실패할 경우 에러 상태를 반환하여 재로그인을 유도
 *
 * @example
 * // JWT 콜백에서 자동 호출
